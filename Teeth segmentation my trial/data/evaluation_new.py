...
def evaluate(device, model, data_loader, criterion, metric,newmymetric):
    ...
    with torch.no_grad():
        ...
        for batch_data in tqdm.tqdm(data_loader, total=len_dl):
            ...
            loss =1- newmymetric(outputs,targets)
...
if __name__ == "__main__":
    ...
    # evaluate model
    test_loss, test_mydice = evaluate(device, model, test_ds, criterion, metric,newmymetric)
    

    print(f"Test: {1-np.nanmean(test_mydice, 0):.4f} loss,  {np.nanmean(test_mydice, 0):.4f} MY dice mean.")
